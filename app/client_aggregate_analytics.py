...
class ClientsAggregateAnalytics:
    ...
    def GetMetrics(self,client,date,business,product,identifier):
        ...
        #     return df_rev[(df_rev['Client']==client) & (df_rev['Date']==date)&
        #     (df_rev['Business Line']==business) & (df_rev['Product']==product) 
        #      ]["Net Amount ($)"].sum()
        
        # if url == "Corporate Loyalty.LBMS.metrics.gross_revenues":
        #     return df_rev[(df_rev['Client']==client) & (df_rev['Date']==date)&
        #     (df_rev['Business Line']==business) & (df_rev['Product']==product) 
        #      ]["Gross Amount ($)"].sum()

        # if url == "Corporate Loyalty.LBMS.metrics.take_rate":
        ...
        # if url == "Corporate Loyalty.LBMS.metrics.net_revenue_per_active_user":
        #     net_revenue = self.GetMetrics(client,date,business,product,"net_revenues")
        #     return net_revenue / self.GetMetrics(client,date,business,product,"active_users")

        # if url == "Corporate Loyalty.LBMS.metrics.accrual_engagement_rate":       
        ...
        # This is a first order metrics stored in the main_frame
